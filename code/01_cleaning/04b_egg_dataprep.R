...
#get dataframe that describes some metadata on the 
#first observation to each nest
...
egg <- data %>%
  ...
  group_by(Nest_ID, Year_located, Project_ID, 
           Transect_ID2, Trt_cat, Type, Initiation_date,
  ...
  filter(No_eggs != 0) %>%
  filter(No_eggs != -Inf) %>%
  mutate(Month = '05') %>% #set month = 5 as the month that earliest nests start
...
#what are egg numbers?
egg %>%
  filter(Type %in% c("egg")) %>%
  ggplot() +
  geom_histogram(aes(x = No_eggs)) +
  labs(x = "Number of eggs", y = "Number of nests")
...
write.csv(egg2, here("data_outputs",
               "01_cleaning",
               "03_egg_production",
