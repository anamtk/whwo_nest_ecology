...
(pp_check_eggs <- ggplot() +
  #graph the simulated data
  geom_density(data = yrep, aes(x = No_young, group = Iteration, fill = type), 
               alpha = 0.2) +
  geom_density(data = y1, aes(x = No_young, fill = type), alpha = 0.5))

# Observed vs. predicted  --------------------------------------

y2 <- y1 %>%
  rename('observed' = 'No_young') %>%
...
(ob_pre <- ggplot(mean, aes(x = observed, y = predicted)) +
    ...
    geom_abline(slope = 0.70, intercept = 0.71) +
...
(eggs_GOF <- pp_check_eggs + ob_pre +
  plot_annotation(tag_levels = "A",
                  title = "Egg survival model GOF"))
...
resid <- model$mean$resid %>%
  ...
  left_join(egg_s_data, by = c("Row_num"))
...
summary(mod)
pairs(emmeans(mod, 'Project_ID'))


(transect_box <- ggplot(b0.transect.df, aes(x = Project_ID, y = median)) +
  geom_boxplot() +
  labs(y = "Transect-level error") +
  annotate(geom = "text",
           x = 1, y = 2.5,
           label = "*",
           size = 7) +
...
(eggs_resid <- resid_site + (transect_errors/transect_box) +
  plot_layout(ncol = 2) +
  plot_annotation(tag_levels = "A",
                  title = "Egg survival residuals and errors by forest"))
