model{
  ...
  ## Egg survival model for White-headed Woodpecker
  # Ana Miller-ter Kuile
  # April 1, 2022
  
  # this is a binomial model of the number of the number of nestlings
  # based on the probability of surviving the egg stage and
  # the number of initial eggs in each nest. 
  
  # egg survival is a function of the same variables as 
  ...
  # - Bayesian p-values for all covariate parameters
  
  #-------------------------------------## 
  
  #-------------------------------------## 
  #Likelihood of egg binomial model
  #-------------------------------------## 
  for(i in 1:n.nests){
    
    # observed nestlings is a binomial distribution
    # around hatch/survival probability and the number
    # of initial eggs in the nest
    y[i] ~ dbin(p[i], N.eggs[i])
    ...
    logit(p[i]) <- 
      ...
      b[16]*LandHa[i] +
      #interactions
      b[17]*Trees2550[i]*PercPonderosa[i] +
      b[18]*Trees50[i]*Tmax[i] +
      b[19]*Trees2550[i]*Tmax[i] +
      b[20]*LandHa[i]*LandBu[i] +
      b[21]*Tmax[i]*PPT[i]
