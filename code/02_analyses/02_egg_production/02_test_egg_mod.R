...
model <- here("code", "02_analyses", 
              "02_egg_production",
              "jags", 
              "model_egg_num_pvalue.R")


egg_mod <- jagsUI::jags(data = data,
                            inits = NULL,
                            model.file = model,
...
raf_egg <- raftery.diag(egg_mod$samples)

names <- rownames(raf_egg[[1]]$resmatrix)
ch1 <- raf_egg[[1]]$resmatrix[,2]
ch2 <- raf_egg[[2]]$resmatrix[,2]
ch3 <- raf_egg[[3]]$resmatrix[,2]
...
# <dbl>         <dbl>  <dbl>
#   1        23348.        32610. 111020

bu1 <- raf_egg[[1]]$resmatrix[,1]
bu2 <- raf_egg[[2]]$resmatrix[,1]
bu3 <- raf_egg[[3]]$resmatrix[,1]
...
b0.transect <- egg_mod$mean$b0.transect
sig.transect <- egg_mod$mean$sig.transect
b0.year <- c(egg_mod$mean$b0.year[1:9], NA)
sig.year <- egg_mod$mean$sig.year
b0 <- egg_mod$mean$b0
b1 <- c(NA, egg_mod$mean$b1[2:length(egg_mod$mean$b1)])
b <- egg_mod$mean$b
