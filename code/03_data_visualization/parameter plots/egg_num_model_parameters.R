...
egg_number_df <- egg_number %>%
  ...
  mutate(category = case_when(parameter %in% c('b1[2]',
                              ...
                              parameter %in% c("b[10]", "b[11]") ~ "Landscape management (314 ha)",
                              parameter == "b[2]" ~ "Nest habitat",
                              parameter %in% c('b[3]', 'b[4]', 
                                               'b[5]') ~ "Local habitat (0.4 ha)",
                              parameter %in% c("b[6]") ~ "Climate (27 ha)",
                              parameter %in% c("b[7]", "b[8]", "b[9]") ~ "Landscape habitat (314 ha)",
                              parameter %in% c("b[12]", "b[13]",
                                               "b[14]", "b[15]",
                                               "b[16]") ~ "Variable interactions",
                              TRUE~ NA_character_)) %>%
  mutate(parameter = case_when(parameter == "b1[2]" ~ "Treatment type: Burn",
                               ...
                               parameter == "b[7]" ~ "Forest patch size coeff. variation",
                               parameter == "b[9]" ~ "Contagion index",
                               parameter == "b[8]" ~ "Proximity of forest patches",
                               parameter == "b[10]" ~ "Percent of landscape harvested",
                               parameter == "b[11]" ~ "Percent of landscape burned",
                               parameter == "b[12]" ~ "Large tree density*perc. ponderosa",
                               parameter == "b[13]" ~ "Small tree density*perc. ponderosa",
                               parameter == "b[14]"~ "Large tree density*maximum temperature",
                               parameter == "b[15]" ~ "Small tree density*maximum temperature",
                               parameter == "b[16]" ~ "Percent of landscape harvested*percent burned",
                               TRUE ~ parameter)) %>%
