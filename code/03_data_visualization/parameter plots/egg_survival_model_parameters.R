...
egg_survival_df <- egg_survival %>%
  ...
  mutate(parameter = case_when(parameter == "b1TrtID[2]" ~ "Treatment type: Harvest",
                               ...
                               parameter == "b[17]" ~ "Small tree density*perc. ponderosa",
                               parameter == "b[18]"~ "Large tree density*maximum temperature",
                               parameter == "b[19]" ~ "Small tree density*maximum temperature",
                               parameter == "b[20]" ~ "Percent of landscape harvested*percent burned",
                               parameter == "b[21]" ~ "Maximum temperature*precipitation",
                               TRUE ~ parameter)) %>%
