...
egg_survival <- as.data.frame(egg_s$quantiles) %>%
  rownames_to_column(var = "parameter") %>%
  filter(str_detect(parameter, "b")) %>%
  filter(!str_detect(parameter, "b0|z")) 

parameters <- egg_survival$parameter

# Get p-values per model --------------------------------------------------

egg_survival_ps <- as.data.frame(egg_s$statistics) %>%
  dplyr::select(Mean) %>%
...
egg_survival_df <- egg_survival %>%
  left_join(egg_survival_ps, by = "parameter") %>%
  filter(!parameter %in% c("b2SpeciesID[1]", "b1TrtID[1]")) %>%
  ...
  mutate(parameter = case_when(parameter == "b1TrtID[2]" ~ "Treatment type: Harvest",
                               ...
                               parameter == "b[14]" ~ "Total number forest patches",
                               parameter == "b[15]" ~ "Forest patch size CV",
                               parameter == "b[16]" ~ "Contagion Index",
                               parameter == "b[17]" ~ "Landscape Patch Index",
                               parameter == "b[18]"~ "Landscape perc. Burned",
                               parameter == "b[19]" ~ "Landscape perc. Harvested",
                               parameter == "b[20]" ~ "Small trees * Percent ponderosa",
                               parameter == 'b[21]' ~ "Large trees * temperature",
                               parameter == "b[22]" ~ "Small trees * temperature",
                               parameter == "b[23]" ~ "Perc. harvest * perc. burned",
                               parameter == "b[24]" ~ "Temperature * precipitation",
                               TRUE ~ parameter)) %>%
